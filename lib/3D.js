...
function initGraphics() {
	...
	console.log('svg', svg)
	$.LoadingOverlay("show", {image:'./panny-obj/rca.svg', zIndex:200});
	NProgress.start();
	...
	objLoader.load( currentObject.uri, function ( readObj ) {
		...
		readObj && readObj.traverse( function ( child ) {
			if ( child instanceof THREE.Mesh) {
				...
				parent.add( second );
				parent.add( child );
				parent.scale.set(currentObject.scale[0], currentObject.scale[1], currentObject.scale[2])
...
function createBallsWithPhysics( mesh, collider, mass, pos, quat, material, index ) {
	...
	if (mesh.userData.customScale) {
		mesh.scale.set(mesh.userData.customScale[0], mesh.userData.customScale[1], mesh.userData.customScale[2])
	}
