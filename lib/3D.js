...
function importObjects(index, manager) {
	...
	objLoader.load( objectsURI[index], function ( readObj ) {
		// fbx = readObj;
		readObj && readObj.traverse( function ( child ) {
			console.log('child', child)
			if ( child instanceof THREE.Mesh) {
				child.material = new THREE.MeshPhysicalMaterial({envMap: hervisions_hdri, roughness: 0, metalness: 1, color: 0x00FFFF});
				var second = child.clone();
				second.material = new THREE.MeshPhysicalMaterial({envMap: hervisions_hdri, roughness: 0, metalness: 1, color: 0x00FFFF});
