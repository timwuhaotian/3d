...
function importObjects(index, manager) {
  ...
  const isObj = currentObject.uri.indexOf('.glb') < 0;
  const reader = isObj ? objLoader : glbLoader;
  reader.load(currentObject.uri, function (readObj) {
    const travelsalLayer = isObj ? readObj : readObj.scene;
    travelsalLayer &&
      travelsalLayer.traverse(function (child) {
